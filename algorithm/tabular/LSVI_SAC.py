...
class LSVI_SAC(nn.Module):
	...
	def actor_step(self,batch):
		...
		if self.action_num is None:
			...
			q1,q2=self.critic(state,action)
			q_min=torch.min(q1,q2).reshape(self.batch_size,self.action_num)

			actor_loss=(dist.probs*(self.alpha.detach()*torch.log(dist.probs)-q_min.detach())).sum(dim=-1).mean()
	...
	def critic_step(self,batch):
		...
		with torch.no_grad():
			...
			if self.action_num is None:
				...
				q1_next,q2_next=self.critic_target(state_next,action_next)
				q_min=torch.min(q1_next,q2_next).reshape(self.batch_size,self.action_num)
				q_next=(dist.probs*(q_min-self.alpha*torch.log(dist.probs))).sum(dim=-1)

			q_nxt=torch.clamp(q_next,max=self.rho) if self.rho is not None else q_next
			q_target=reward+(1-done)*self.gamma*q_nxt
		...
		return {"q1":q1.mean().item(),"q2":q2.mean().item(),"q_max":q_next.max().item(),"q1_loss":q1_loss.item(),"q2_loss":q2_loss.item()}
