...
class LSVI():
	...
	def __init__(self,env,state_dim,action_dim,feature_dim,action_space,K,beta,lamda,gamma,tau,epsilon,rho=None):
		...
		self.at=0
		self.state_history=np.zeros((K,state_dim))  # store state
		self.action_history=np.zeros((K,action_dim))  # store action
		self.state_next=np.zeros((K,state_dim))  # store next state
	...
	def insert(self,state,action,phi,reward,state_next,phi_next,done):
		self.state_history[self.at]=state
		self.action_history[self.at]=action
	...
	def update_feature(self,feature_func):
		self.phi_matrix[:self.at]=feature_func(self.state_history[:self.at],self.action_history[:self.at])
		self.phi_next[:self.at]=np.stack([feature_func(self.state_next[:self.at],np.repeat([a],self.at,axis=0)) for a in self.action_space],axis=1)
		self.Lambda=self.lamda*np.eye(self.feature_dim)+self.phi_matrix[:self.at].T@self.phi_matrix[:self.at]
		self.Lambda_dec=np.linalg.cholesky(self.Lambda)
