...
class TD3(nn.Module):
	...
	def actor_step(self,batch):
		...
		actor_loss=-self.critic(state,action)[0].mean()
	...
	def critic_step(self,batch):
		...
		return {"q1":q1.mean().item(),"q2":q2.mean().item(),"q_max":q_next.max().item(),"frac":(q_next>self.rho).float().mean().item(),"q1_loss":q1_loss.item(),"q2_loss":q2_loss.item()}
